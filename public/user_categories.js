...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      ...
      // Se tipo não informado, retorna todas as categorias do usuário
      let query, params;
      if (tipo) {
        query = `SELECT id, nome, tipo FROM user_categories WHERE user_id = $1 AND tipo = $2 ORDER BY tipo, nome`;
        params = [userId, tipo];
      } else {
        query = `SELECT id, nome, tipo FROM user_categories WHERE user_id = $1 ORDER BY tipo, nome`;
        params = [userId];
      }
